...
esp_err_t esp_netif_add_to_list(esp_netif_t *netif)
{
    ...
    ++s_esp_netif_counter;
    ESP_LOGD(TAG, "%s netif added successfully (total netifs: %zu)", __func__, s_esp_netif_counter);
...
esp_err_t esp_netif_remove_from_list(esp_netif_t *netif)
{
    ...
    SLIST_FOREACH(item, &s_head, next) {
        if (item->netif == netif) {
            ...
            --s_esp_netif_counter;
            ESP_LOGD(TAG, "%s netif successfully removed (total netifs: %zu)", __func__, s_esp_netif_counter);
            free(item);
            esp_netif_list_unlock();
            return ESP_OK;
        }
