#include "roo_testing/microcontrollers/esp32/fake_esp32_espnow.h"
...
bool EspNowInterface::send(const uint8_t* addr, const void* data, size_t len) {
...
void EspNowInterface::attachDevice(FakeEspNowDevice& device) {
...
void EspNowInterface::incoming(uint64_t peer, const void* data, size_t len) {
  if (len == 0) return;
  roo_testing::lock_guard<roo_testing::mutex> guard(mutex_);
  std::unique_ptr<uint8_t[]> payload(new uint8_t[len]);
  memcpy(payload.get(), data, len);
  inbox_.push(Packet{
      .peer_mac = peer, .size = (int)len, .payload = std::move(payload)});
  nonempty_.notify_all();
}

void EspNowInterface::setSendCb(OnSentCb send_cb) {
  roo_testing::lock_guard<roo_testing::mutex> guard(mutex_);
  on_sent_cb_ = std::move(send_cb);
}

void EspNowInterface::setRecvCb(OnRecvCb recv_cb) {
  roo_testing::lock_guard<roo_testing::mutex> guard(mutex_);
  on_recv_cb_ = std::move(recv_cb);
}

void EspNowInterface::processInbox() {
  Packet packet;
  OnRecvCb cb;
  while (true) {
    {
...
void process_inbox_fn(EspNowInterface* interface) {
  interface->processInbox();
}

void EspNowInterface::start() {
