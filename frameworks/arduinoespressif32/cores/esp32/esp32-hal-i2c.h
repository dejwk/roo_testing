...
typedef void * EventGroupHandle_t; // from event_groups.h
...
// The actual fakes, implemented by the fake bus library.
uint8_t i2cFakeWrite(uint8_t i2c_num, uint16_t address, uint8_t* buff, uint16_t size, bool sendStop, uint16_t timeOutMillis);
uint8_t i2cFakeRead(uint8_t i2c_num, uint16_t address, uint8_t* buff, uint16_t size, bool sendStop, uint16_t timeOutMillis);
