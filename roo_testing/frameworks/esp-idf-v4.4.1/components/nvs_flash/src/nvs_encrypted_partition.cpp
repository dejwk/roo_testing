...
esp_err_t NVSEncryptedPartition::read(size_t src_offset, void* dst,
                                      size_t size) {
  /** Currently upper layer of NVS reads entries one by one even for variable
   * size multi-entry data types. So length should always be equal to size of an
   * entry.*/
  if (size != sizeof(Item)) return ESP_ERR_INVALID_SIZE;

  // read data
  esp_err_t read_result =
      esp_partition_read(mESPPartition, src_offset, dst, size);
  ...
  // if (mbedtls_aes_crypt_xts(&mDctxt, MBEDTLS_AES_DECRYPT, size, data_unit,
...
esp_err_t NVSEncryptedPartition::write(size_t addr, const void* src,
                                       size_t size) {
  ...
  /* Use relative address instead of absolute address (relocatable), so that
   * host-generated encrypted nvs images can be used*/
