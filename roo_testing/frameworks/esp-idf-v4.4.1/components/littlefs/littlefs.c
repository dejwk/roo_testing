...
esp_err_t format_from_efs(esp_littlefs_t *efs)
{
    ...
    if(efs->cache_size > 0){
        ...
        was_mounted = true;
        res = LFS_ERR_OK;// lfs_unmount(efs->fs);
        if(res != LFS_ERR_OK){
...
#endif
    {
        uint32_t flash_page_size = 1;//g_rom_flashchip.page_size;
...
static ssize_t vfs_littlefs_write(void* ctx, int fd, const void * data, size_t size) {
    ...
    file = efs->cache[fd];
    res = LFS_ERR_OK;// lfs_file_write(efs->fs, &file->file, data, size);
...
static ssize_t vfs_littlefs_read(void* ctx, int fd, void * dst, size_t size) {
    ...
    file = efs->cache[fd];
    res = LFS_ERR_OK;// lfs_file_read(efs->fs, &file->file, dst, size);
    sem_give(efs);
...
static ssize_t vfs_littlefs_pwrite(void *ctx, int fd, const void *src, size_t size, off_t offset)
{
    ...
    off_t old_offset = 0;// lfs_file_seek(efs->fs, &file->file, 0, SEEK_CUR);
    ...
    res = 0;//lfs_file_seek(efs->fs, &file->file, offset, SEEK_SET);
    if (res < (off_t)0)
        goto exit;

    /* Write out the data.  */
    res = LFS_ERR_OK;// lfs_file_write(efs->fs, &file->file, src, size);

    /* Now we have to restore the position.  If this fails we have to
     return this as an error. But if the writing also failed we
     return writing error.  */
    save_res = 0;//lfs_file_seek(efs->fs, &file->file, old_offset, SEEK_SET);
...
#endif
    res = LFS_ERR_OK;// lfs_file_close(efs->fs, &file->file);
    if(res < 0){
        errno = lfs_errno_remap(res);
        sem_give(efs);
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
...
static off_t vfs_littlefs_lseek(void* ctx, int fd, off_t offset, int mode) {
    ...
    file = efs->cache[fd];
    res = 0;// lfs_file_seek(efs->fs, &file->file, offset, whence);
    sem_give(efs);
...
static int vfs_littlefs_fstat(void* ctx, int fd, struct stat * st) {
    ...
    file = efs->cache[fd];
    res = 0;//lfs_stat(efs->fs, file->path, &info);
    if (res < 0) {
        errno = lfs_errno_remap(res);
        sem_give(efs);
...
static int vfs_littlefs_stat(void* ctx, const char * path, struct stat * st) {
    ...
    sem_take(efs);
    res = 0;//lfs_stat(efs->fs, path, &info);
    if (res < 0) {
        errno = lfs_errno_remap(res);
        sem_give(efs);
...
static int vfs_littlefs_rename(void* ctx, const char *src, const char *dst) {
    ...
    res = 0;//lfs_rename(efs->fs, src, dst);
    if (res < 0) {
        errno = lfs_errno_remap(res);
        sem_give(efs);
...
static DIR* vfs_littlefs_opendir(void* ctx, const char* name) {
    ...
    sem_take(efs);
    res = 0;//lfs_dir_open(efs->fs, &dir->d, dir->path);
    sem_give(efs);
    if (res < 0) {
        errno = lfs_errno_remap(res);
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
...
static int vfs_littlefs_closedir(void* ctx, DIR* pdir) {
    ...
    sem_take(efs);
    res = 0;//lfs_dir_close(efs->fs, &dir->d);
    sem_give(efs);
    if (res < 0) {
        errno = lfs_errno_remap(res);
#ifndef CONFIG_LITTLEFS_USE_ONLY_HASH
...
static int vfs_littlefs_readdir_r(void* ctx, DIR* pdir,
    ...
    do{ /* Read until we get a real object name */
        res = 0;//lfs_dir_read(efs->fs, &dir->d, &info);
...
static void vfs_littlefs_seekdir(void* ctx, DIR* pdir, long offset) {
    ...
    if (offset < dir->offset) {
        ...
        sem_take(efs);
        res = 0;//lfs_dir_rewind(efs->fs, &dir->d);
        sem_give(efs);
        if (res < 0) {
            errno = lfs_errno_remap(res);
...
static int vfs_littlefs_rmdir(void* ctx, const char* name) {
    ...
    /* Error Checking */
    sem_take(efs);
    res = 0;//lfs_stat(efs->fs, name, &info);
    if (res < 0) {
        errno = lfs_errno_remap(res);
        sem_give(efs);
...
static ssize_t vfs_littlefs_truncate( void *ctx, const char *path, off_t size )
{
    ...
    file = efs->cache[fd];
    res = 0;//lfs_file_truncate( efs->fs, &file->file, size );
    sem_give(efs);
...
#endif
    res = 0;//lfs_file_sync(efs->fs, &file->file);
    return res;
}
...
{
    int res;
    res = 0;//lfs_setattr(efs->fs, path, ESP_LITTLEFS_ATTR_MTIME,
            // &t, sizeof(t));
    if( res < 0 ) {
        errno = lfs_errno_remap(res);
