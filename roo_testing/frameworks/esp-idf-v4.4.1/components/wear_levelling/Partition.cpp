...
size_t Partition::chip_size() { return this->partition->size; }
...
esp_err_t Partition::erase_range(size_t start_address, size_t size) {
  esp_err_t result =
      esp_partition_erase_range(this->partition, start_address, size);
  if (result == ESP_OK) {
    ESP_LOGV(TAG,
...
size_t Partition::sector_size() { return SPI_FLASH_SEC_SIZE; }
