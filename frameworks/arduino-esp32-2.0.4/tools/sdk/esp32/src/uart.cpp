...
int uart_write_bytes_with_break(uart_port_t uart_num, const void* src,
                                size_t size, int brk_len) {
  return uart_write_bytes(uart_num, src, size);
}

int uart_read_bytes(uart_port_t uart_num, void* buf, uint32_t length,
                    TickType_t ticks_to_wait) {
  return FakeEsp32().uart(uart_num).read(reinterpret_cast<uint8_t*>(buf), length);
}
