...
unsigned long micros() {
  return static_cast<unsigned long>(system_time_get_micros());
}
unsigned long millis() { return micros() / 1000UL; }
void delay(uint32_t milliseconds) {
  system_time_delay_micros(static_cast<uint64_t>(milliseconds) * 1000);
}
void delayMicroseconds(uint32_t microseconds) {
  if (xPortInIsrContext()) {
    system_time_busy_wait_micros(microseconds);
    return;
  }
  system_time_delay_micros(microseconds);
}
...
void analogWriteFrequency(uint8_t pin, uint32_t frequency) {
  ...
  if (ledcReadFreq(pin))
    ledcChangeFrequency(pin, frequency,
                        analog_resolution[pin] ? analog_resolution[pin] : 8);
}
void analogWriteResolution(uint8_t pin, uint8_t bits) {
  ...
  if (ledcReadFreq(pin))
    ledcChangeFrequency(
        pin, analog_frequency[pin] ? analog_frequency[pin] : 1000, bits);
}
...
BaseType_t xTaskCreateUniversal(TaskFunction_t task, const char* const name,
                                ...
                                void* const parameter, UBaseType_t priority,
                                TaskHandle_t* const handle, const BaseType_t) {
...
bool removeApbChangeCallback(void* arg, apb_change_cb_t callback) {
  std::lock_guard<std::mutex> lock(callback_mutex);
  auto it =
      std::find_if(apb_callbacks.begin(), apb_callbacks.end(),
...
bool setCpuFrequencyMhz(uint32_t frequency) {
  ...
  std::lock_guard<std::mutex> lock(callback_mutex);
  for (const auto& entry : apb_callbacks)
    entry.callback(entry.arg, APB_BEFORE_CHANGE, old_apb, old_apb);
  cpu_frequency_mhz = frequency;
  for (const auto& entry : apb_callbacks)
    entry.callback(entry.arg, APB_AFTER_CHANGE, old_apb, getApbFrequency());
  return true;
}
const char* getSupportedCpuFrequencyMhz(uint8_t) {
  return "240,160,80,40,20,10";
}
...
uint32_t getApbFrequency() {
  return std::min<uint32_t>(cpu_frequency_mhz, 80) * 1000000U;
}
